...
**Purpose**: Demonstrates how to use Entra ID for **outbound authentication** where agents access external resources (Microsoft OneNote) on behalf of authenticated users.
...
- Building agents that create and manage OneNote notebooks

**Key Integration Pattern**: 
- User authenticates with Entra ID
- Agent receives delegated permissions to access OneNote API
- Agent performs actions on user's behalf
...
**Purpose**: Shows how to use Entra ID for **inbound authentication** to protect agent endpoints, ensuring only authenticated users can invoke agents.
...
**Key Integration Pattern**:
- Users must authenticate with Entra ID before accessing agents
