...
| File | Location | Purpose | Generated By | Should Commit? |
|------|----------|---------|--------------|----------------|
| `.env` | Tutorial root | Environment variables for local development | Manual/User | No (in .gitignore) |
| `.bedrock_agentcore.yaml` | Tutorial root | AgentCore Runtime configuration cache | bedrock-agentcore-starter-toolkit | No (in .gitignore) |
| `.deployment_metadata.json` | `scripts/` | Deployment information (agent ID, ARN, etc.) | `deploy_agent.py` | No (in .gitignore) |
| `.env.backup` | `scripts/` | Backup of previous .env file | `setup_braintrust.sh` | No (in .gitignore) |
| `cloudwatch-urls.txt` | `scripts/` | CloudWatch dashboard and X-Ray URLs | `setup_cloudwatch.sh` | No (in .gitignore) |
| `xray-permissions.json` | `scripts/` | IAM policy for X-Ray permissions | `setup_cloudwatch.sh` | No (in .gitignore) |
| `Dockerfile` | Tutorial root | Docker container definition | bedrock-agentcore-starter-toolkit | No (auto-generated) |
| `.dockerignore` | Tutorial root | Docker build exclusions | bedrock-agentcore-starter-toolkit | No (auto-generated) |
...
### `cloudwatch-urls.txt`
**Location:** `scripts/cloudwatch-urls.txt`

**Purpose:** Contains direct URLs to CloudWatch dashboards, X-Ray traces, and log groups for easy access to observability tools.

**Generated By:** `scripts/setup_cloudwatch.sh` after creating CloudWatch resources

**Typical Contents:**
```
CloudWatch Dashboard:
https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=AgentCore-Observability-Demo

X-Ray Service Map:
https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#xray:service-map
...
**Usage:**
- Reference file for quick access to observability dashboards
- Displayed by `setup_all.sh` at completion
- Used for documentation and user guidance

**Important Notes:**
- Listed in `.gitignore` (contains account-specific URLs)
- Safe to delete - just convenience links
- Can be regenerated by running `setup_cloudwatch.sh`

**Generation Process:**
```bash
# In setup_cloudwatch.sh
cat > "$SCRIPT_DIR/cloudwatch-urls.txt" << EOF
CloudWatch Dashboard:
https://console.aws.amazon.com/cloudwatch/home?region=$REGION#dashboards:name=$DASHBOARD_NAME

X-Ray Service Map:
https://console.aws.amazon.com/cloudwatch/home?region=$REGION#xray:service-map
...
EOF
```
...
**Location:** `scripts/xray-permissions.json`

**Purpose:** Contains IAM policy JSON for X-Ray permissions needed by the agent execution role to send traces to CloudWatch X-Ray.

**Generated By:** `scripts/setup_cloudwatch.sh` when configuring X-Ray tracing
...
**Usage:**
- Used by `setup_cloudwatch.sh` to attach X-Ray permissions to agent role
- Reference document for required permissions
- Can be used for manual IAM policy creation

**Important Notes:**
- Listed in `.gitignore` (account-specific)
- Safe to delete - will be regenerated if needed
- Contains standard X-Ray permissions policy

**Generation Process:**
```bash
# In setup_cloudwatch.sh
cat > "$SCRIPT_DIR/xray-permissions.json" << 'EOF'
{
  "Version": "2012-10-17",
  "Statement": [...]
}
...
### During CloudWatch Setup (`scripts/setup_cloudwatch.sh`)
1. `cloudwatch-urls.txt` - Created with dashboard URLs
2. `xray-permissions.json` - Created with IAM policy
...
3. `cloudwatch-urls.txt` - Optionally deleted
4. `xray-permissions.json` - Optionally deleted
