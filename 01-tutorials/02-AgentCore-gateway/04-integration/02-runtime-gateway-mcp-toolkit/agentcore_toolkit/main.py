...
class AgentCoreToolkit:
    ...
    def _check_required_files(self, runtime_config):
        ...
        required_files = [runtime_config['entrypoint'], runtime_config['requirements_file']]
    ...
    def setup_gateway_cognito(self):
        ...
        scopes = [{"ScopeName": scope['name'], "ScopeDescription": scope['description']} 
                 for scope in self.config['scopes']]
        utils.get_or_create_resource_server(cognito, gw_user_pool_id, 
                                          derived_names['resource_server_id'], 
                                          derived_names['resource_server_name'], scopes)
        
        # Create client
        scope_names = [f"{derived_names['resource_server_id']}/{scope['name']}" 
                      for scope in self.config['scopes']]
        gw_client_id, gw_client_secret = utils.get_or_create_m2m_client(
            cognito, gw_user_pool_id, derived_names['client_name'], 
            derived_names['resource_server_id'], scope_names)
    ...
    def setup_runtime_cognito(self, runtime_config):
        ...
        scopes = [{"ScopeName": scope['name'], "ScopeDescription": scope['description']} 
                 for scope in self.config['scopes']]
        utils.get_or_create_resource_server(cognito, rt_user_pool_id, 
                                          derived_names['resource_server_id'], 
                                          derived_names['resource_server_name'], scopes)
        ...
        rt_client_id, rt_client_secret = utils.get_or_create_m2m_client(
            cognito, rt_user_pool_id, derived_names['client_name'], 
            derived_names['resource_server_id'], scope_names)
    ...
    def create_gateway_target(self, gateway_info, runtime_info, runtime_cognito, target_config):
        ...
        cognito_provider_arn = utils.get_or_create_oauth2_credential_provider(self.region, target_config['identity_provider_name'], runtime_cognito)
        ...
        gw_target_info = utils.get_or_create_agentcore_gateway_target(self.region, target_creation_params)
    ...
    def _get_access_token(self, gateway_cognito):
        ...
        try:
            ...
            scope_names = [f"{gateway_cognito['resource_server_id']}/{scope['name']}" for scope in self.config['scopes']]
            scope_string = " ".join(scope_names)
...
def main():
    ...
    parser.add_argument('--runtime-configs', required=True, 
