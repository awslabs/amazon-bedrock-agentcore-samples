...
def delete_stack(stack_name, region, cf_client, wait=True):
    ...
    try:
        ...
        if stack_status == 'DELETE_FAILED':
            print(f"⚠️  Stack is in DELETE_FAILED state. Will attempt to retry deletion...")
    ...
    try:
        ...
        print(f"✅ Delete request submitted successfully")
    ...
    try:
        while True:
            ...
            if elapsed > max_wait_seconds:
                ...
                print(f"   Check AWS Console for current status")
                return False
        ...
        print(f"   Stack deletion will continue in the background")
        return False
...
def _print_deletion_errors(stack_name, cf_client):
    ...
    try:
        ...
        failed_events = [
            event for event in events['StackEvents']
            if 'FAILED' in event.get('ResourceStatus', '')
        ]
