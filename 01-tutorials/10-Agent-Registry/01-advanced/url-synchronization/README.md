...
cp = boto3.client("bedrock-agentcore-registry-control", region_name="us-west-2")
...
- Registry created after the URL sync feature deployment (older registries lack the workload identity needed for credential resolution)
- For OAuth sync: an OAuth2 credential provider configured in AgentCore
- For IAM sync: an IAM role with `bedrock-agentcore:InvokeGateway` permission and trust policy for `bedrock-agentcore.amazonaws.com`
- Additional IAM permissions for the caller — see [IAM_PERMISSIONS.md](../../IAM_PERMISSIONS.md)
