...
```bash
aws bedrock-agentcore-registry-control list-registries \
...
- `strands-agents >= 1.23.0` (ToolProvider interface)
- AWS credentials with `bedrock-agentcore-registry:SearchRegistryRecords` permission
- For MCP tools: a Gateway URL and token function
- For A2A tools: `bedrock-agentcore:InvokeAgentRuntime` permission
