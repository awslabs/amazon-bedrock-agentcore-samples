...
def setup_cognito_user_pool(userpool_name="MCPServerPool"):
    try:
        # Create User Pool
        user_pool_response = cognito_client.create_user_pool(
            PoolName=userpool_name, Policies={"PasswordPolicy": {"MinimumLength": 8}}
        )
...
def get_bearer_token(client_id, username="testuser", password="MyPassword123!"):
    try:
        auth_response = cognito_client.initiate_auth(
            ClientId=client_id,
            AuthFlow="USER_PASSWORD_AUTH",
...
def delete_cognito_user_pool(pool_id):
    try:
...
def create_agentcore_role(agent_name):
    ...
    try:
        ...
        print(f"Error attaching role policy: {e}")

    return agentcore_iam_role


def delete_agentcore_role(agent_name):
    agentcore_role_name = f"agentcore-{agent_name}-role"
    ...
    try:
        ...
        iam_client.delete_role(RoleName=agentcore_role_name)
        print(f"Successfully deleted role: {agentcore_role_name}")
        return True

    except Exception as e:
        print(f"Error deleting agentcore role: {e}")
        return False
