...
async def _setup_session(stream, prompt_name):
    ...
    await _send(stream, {"event": {"sessionStart": {
        "inferenceConfiguration": {"maxTokens": 1024, "topP": 0.9, "temperature": 0.7}
    }}})

    # 2. Configure audio output (Nova Sonic -> browser)
    await _send(stream, {"event": {"promptStart": {
        "promptName": prompt_name,
        "audioOutputConfiguration": {
            **_AUDIO_FORMAT, "sampleRateHertz": OUTPUT_SAMPLE_RATE, "voiceId": VOICE_ID,
        },
    }}})

    # 3. Send system prompt
    system_content = str(uuid.uuid4())
    await _send(stream, {"event": {"contentStart": {
        "promptName": prompt_name, "contentName": system_content,
        "type": "TEXT", "interactive": True, "role": "SYSTEM",
        "textInputConfiguration": {"mediaType": "text/plain"},
    }}})
    await _send(stream, {"event": {"textInput": {
        "promptName": prompt_name, "contentName": system_content,
        "content": SYSTEM_PROMPT,
    }}})
    await _send(stream, {"event": {"contentEnd": {
        "promptName": prompt_name, "contentName": system_content,
    }}})
...
async def _start_audio_input(stream, prompt_name, audio_content_name):
    ...
    await _send(stream, {"event": {"contentStart": {
        ...
        "audioInputConfiguration": {
            **_AUDIO_FORMAT, "sampleRateHertz": INPUT_SAMPLE_RATE,
        },
    }}})
...
async def run_session(audio_in, audio_out, region, pc_id):
    ...
    try:
        ...
        while True:
            ...
            await _send(stream, {"event": {"audioInput": {
                "promptName": prompt_name,
