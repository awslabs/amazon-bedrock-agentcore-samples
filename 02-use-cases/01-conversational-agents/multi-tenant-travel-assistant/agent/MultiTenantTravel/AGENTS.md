...
│   │   ├── agentcore.ts    # AgentCoreProjectSpec types, including gateways and MCP tools
...
| `agentcore/agentcore.json` (gateways) | `agentcore/.llm-context/agentcore.ts`   | `AgentCoreGateway`      |
| `agentcore/aws-targets.json`          | `agentcore/.llm-context/aws-targets.ts` | `AwsDeploymentTarget[]` |

**The generated index in `.llm-context/README.md` lists a third file, `mcp.ts`, that the pinned CLI does
not emit.** Gateway and MCP types were consolidated into `agentcore.ts` — `AgentCoreGateway`,
`AgentCoreGatewayTarget`, `GatewayTargetType` and `AgentCoreMcpRuntimeTool` are all there. That index is
regenerated by the CLI and marked read-only, so it is left alone rather than corrected in place.
`AgentCoreMcpSpec` is a real type, but it lives in the CDK toolkit package the generated stack imports
from, not in this directory.
