...
class S3TablesLakeFormationIntegration:
    ...
    def ensure_lakeformation_admin(self):
        ...
        existing_principals = [
            a["DataLakePrincipalIdentifier"] for a in existing_admins
        ]
        ...
        try:
            ...
            try:
                ...
                self.iam.update_assume_role_policy(
                    RoleName=self.lf_role_name, PolicyDocument=json.dumps(trust_policy)
                )
    ...
    def register_s3tables_with_lakeformation(self, role_arn: str):
        ...
        try:
            ...
            print(
                f"   Deregistered existing resource to re-register with privileged access"
            )
            time.sleep(2)
        except Exception:
        ...
        try:
            ...
            self.lakeformation.register_resource(
                ResourceArn=resource_arn, RoleArn=role_arn, WithFederation=True
            )
    ...
    def run(self):
        ...
        print(
            f"   S3 Tables Resource: arn:aws:s3tables:{self.region}:{self.account_id}:bucket/*"
        )
