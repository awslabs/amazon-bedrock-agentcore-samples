...
def main():
    ...
    try:
        ...
        m2m_client_id = ssm.get_parameter(
    ...
    try:
        ...
        print(
            f"   SupportedIdentityProviders: {client.get('SupportedIdentityProviders', [])}"
        )
        ...
        print(f"\n🔒 Security:")
        print(
            f"   PreventUserExistenceErrors: {client.get('PreventUserExistenceErrors', 'N/A')}"
        )
        ...
        if not client.get("AllowedOAuthScopes"):
            ...
            print(
                f"   ✅ OAuth scopes configured: {len(client.get('AllowedOAuthScopes', []))} scopes"
            )
        ...
        if not client.get("CallbackURLs"):
            issues.append(
                "⚠️  No CallbackURLs configured (may cause invalid_grant error)"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 400 and "invalid_grant" in response.text:
                    ...
                    print(
                        f"      3. Client not properly configured for client_credentials"
                    )
