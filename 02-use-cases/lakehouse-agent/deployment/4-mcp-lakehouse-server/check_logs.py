...
def main():
    ...
    try:
        ...
        for pattern in patterns:
            try:
                response = logs.describe_log_groups(
                    logGroupNamePrefix=pattern, limit=50
                )
        ...
        if not found_groups:
            ...
            response = logs.describe_log_groups(
                logGroupNamePrefix="/aws/bedrock", limit=50
            )
            ...
            response = logs.describe_log_groups(
                logGroupNamePrefix="/aws/agentcore", limit=50
            )
        ...
        if len(found_groups) == 1:
            ...
            choice = input(
                f"\nEnter number (1-{len(found_groups)}) or press Enter for first: "
            ).strip()
        ...
        try:
            response = logs.filter_log_events(
                logGroupName=selected_group, startTime=start_time, limit=100
            )
