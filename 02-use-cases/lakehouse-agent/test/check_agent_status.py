...
def main():
    ...
    try:
        runtime_arn = ssm.get_parameter(Name="/app/lakehouse-agent/agent-runtime-arn")[
            "Parameter"
        ]["Value"]
    ...
    if found_log_groups:
        ...
        for lg in found_log_groups:
            created_date = datetime.fromtimestamp(lg["created"] / 1000).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            ...
            try:
                ...
                if streams:
                    ...
                    print(
                        f"      ⚠️  No log streams found (agent hasn't been invoked yet)"
                    )
        ...
        print(f"\n   This could mean:")
        print(
            f"   1. The agent hasn't been invoked yet (logs created on first invocation)"
        )
