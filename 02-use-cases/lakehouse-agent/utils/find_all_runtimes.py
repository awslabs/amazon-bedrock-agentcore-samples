...
def main():
    ...
    try:
            ...
            try:
                ...
                if "authorizerConfiguration" in runtime:
                    ...
                    if "customJWTAuthorizer" in auth_config:
                        ...
                        print(f"      Auth: JWT")
                        print(
                            f"         Discovery URL: {jwt_config.get('discoveryUrl')}"
                        )
                        print(
                            f"         Allowed Clients: {jwt_config.get('allowedClients')}"
                        )
                    else:
                            ...
                            if streams:
                                ...
                                for stream in streams:
                                    ...
                                    if last_event:
                                        ...
                                        print(
                                            f"            - {stream_name} (last: {last_event_date})"
                                        )
                            else:
                ...
                if not found_logs:
                    ...
                    print(
                        f"         Expected log group: /aws/bedrock-agentcore/runtime/{runtime_id}"
                    )
        ...
        print(
            f"      aws logs tail '/aws/bedrock-agentcore/runtime/<runtime-id>' --follow"
        )

        print(f"\n📋 To invoke a runtime and generate logs:")
        print(
            f"   1. Use the Streamlit UI: streamlit run streamlit-ui/streamlit_app.py"
        )
