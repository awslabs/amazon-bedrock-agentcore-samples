...
class ChatManager:
    def __init__(self, stack_name: str = "customer-support-vpc"):
    ...
    def _get_nested_stack_name(self, parent_stack_name, logical_resource_id, region):
    ...
    def _get_stack_output(self, stack_name, output_key, region):
    ...
    def _get_agent_arn_from_stack(self):
        """Get agent ARN from CloudFormation stack outputs"""
        region = self._get_aws_region()
        agent_stack_name = self._get_nested_stack_name(
            self.stack_name, "AgentServerStack", region
        )
        runtime_id = self._get_stack_output(agent_stack_name, "AgentRuntimeId", region)
    ...
    def _init_session_state(self):
        ...
        if "agent_arn" not in st.session_state or "region" not in st.session_state:
            try:
                agent_arn, region = self._get_agent_arn_from_stack()
                st.session_state["agent_arn"] = agent_arn
                st.session_state["region"] = region
            except Exception as e:
                st.error(
                    f"❌ Error getting agent configuration from CloudFormation: {e}"
                )
                st.stop()
