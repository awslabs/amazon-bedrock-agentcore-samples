#!/usr/bin/env python3
"""
Video Games Sales Assistant — AgentCore Deployment Script
...
Usage:
  ...
  python deploy.py --region us-west-2 --skip-guardrail
"""
...
AGENT_NAME = "VideoGamesSalesAgent"
MEMORY_NAME = "VideoGamesSalesMemory"
GATEWAY_NAME = "VideoGamesToolsGateway"
GUARDRAIL_NAME = "VideoGamesSalesGuardrail"
ROLE_NAME = "VideoGamesSalesAgentRole"


class VideoGamesSalesDeployer:
    """Deploys the Video Games Sales Assistant to AgentCore."""
    ...
    def create_execution_role(self) -> str:
        ...
        try:
            ...
            resp = self.iam_client.create_role(
                ...
                Description="Execution role for Video Games Sales Agent (AgentCore)",
            )
        ...
        self.iam_client.put_role_policy(
            RoleName=ROLE_NAME,
            PolicyName="VideoGamesSalesAgentPolicy",
    ...
    def create_memory(self) -> str:
        ...
        strategies = [
            {
                StrategyType.SEMANTIC.value: {
                    "name": "Facts",
                    "description": "Extracts facts from video game sales analysis conversations",
        ...
        memory = memory_client.create_memory_and_wait(
            name=MEMORY_NAME,
            description="Memory for video games sales assistant — STM + LTM semantic facts",
    ...
    def deploy_runtime(self, role_arn: str, memory_id: str, env_vars: dict) -> str:
        ...
        resp = self.control_client.create_agent_runtime(
            ...
            environmentVariables=runtime_env,
            description="Video games sales data analyst assistant",
        )
    ...
    def create_gateway(self, db_tools_lambda_arn: str) -> dict:
        ...
        tool_schema = [
            {
                ...
                "description": "Get database schema metadata including table structures, columns, and relationships for video game sales data",
                "inputSchema": {
                    "type": "object",
                    "properties": {},
                    "required": [],
            ...
            },
            {
                ...
                "description": "Execute a read-only SQL query against the video game sales PostgreSQL database",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "sql_query": {
        ...
        if not gateway_id:
            ...
            resp = self.control_client.create_gateway(
                name=GATEWAY_NAME,
                protocolType="MCP",
                description="Gateway for video game sales PostgreSQL tools",
            )
        ...
        try:
            self.control_client.create_gateway_target(
                ...
                name=target_name,
                description="PostgreSQL query tools for video game sales data",
                targetConfiguration={
                    "mcp": {
                        "lambda": {
    ...
    def create_guardrail(self) -> dict:
        ...
        Blocks:
          ...
          - Internal cost/margin data queries
          - Raw customer personal information requests
        """
        logger.info("Creating Bedrock Guardrail: %s", GUARDRAIL_NAME)

        try:
            resp = self.bedrock_client.create_guardrail(
                name=GUARDRAIL_NAME,
                description="Content filtering for video game sales assistant",
                topicPolicyConfig={
                    "topicsConfig": [
                        {
                            ...
                            "examples": [
                                ...
                                "What's the wholesale price we pay?",
                            ],
                            "type": "DENY",
                        },
                        {
                            "name": "RawCustomerPII",
                            "definition": "Requests to expose individual customer personal information such as names, emails, phone numbers, or addresses from the database",
                            "examples": [
                                "Show me customer email addresses",
                                "List all buyer names and phone numbers",
                                "Give me the mailing addresses of customers who bought this game",
                            ],
                            "type": "DENY",
                        },
                    ]
            ...
            logger.info(
                "Guardrail created: %s (version %s)", guardrail_id, guardrail_version
            )
            return {
            ...
            if "ConflictException" in str(type(e)):
                logger.info("Guardrail already exists, skipping creation")
                return {}
            raise
    ...
    def deploy(self, env_vars: dict, skip_guardrail: bool = False) -> dict:
        """Run the full deployment pipeline.

        Args:
            env_vars: Environment variables from CDK outputs (AURORA_RESOURCE_ARN, etc.)
            skip_guardrail: Skip guardrail creation if True

        Returns:
            Dictionary with all deployed resource identifiers
        """
        logger.info("=" * 60)
        logger.info("VIDEO GAMES SALES ASSISTANT — AgentCore Deployment")
        logger.info("=" * 60)
        ...
        if not skip_guardrail:
            guardrail_info = self.create_guardrail()
        ...
        if guardrail_info:
            logger.info("  Guardrail   : %s", guardrail_info.get("guardrail_id"))
...
def main():
    parser = argparse.ArgumentParser(
        description="Deploy Video Games Sales Assistant to AgentCore"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--skip-guardrail",
        action="store_true",
        help="Skip Bedrock Guardrail creation",
    )
    parser.add_argument(
    ...
    deployer = VideoGamesSalesDeployer(region=args.region)
    deployer.deploy(env_vars=env_vars, skip_guardrail=args.skip_guardrail)
