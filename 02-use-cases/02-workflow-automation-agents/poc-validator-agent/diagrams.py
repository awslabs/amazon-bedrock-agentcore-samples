...
with Diagram(
    "POC Validator | AWS Architecture | v1.0",
    filename=os.path.join(_here, "architecture"),
    show=False,
    direction="LR",
    graph_attr={**GRAPH, "nodesep": "0.7", "ranksep": "1.1", "size": "30,18"},
    node_attr=NODE,
    edge_attr=EDGE,
):
    ...
    with Cluster("Amazon Bedrock AgentCore", graph_attr=_C_AGENTCORE):
        runtime = Custom("AgentCore Runtime\n5-phase entrypoint", ICON_RUNTIME)
        memory = Custom("Memory\nSEMANTIC + SUMMARIZATION", ICON_RUNTIME)
...
# Cost: $0.02-0.08/review (Runtime+Bedrock) + near-zero idle (Lambda/DynamoDB/S3 on-demand)
