...
def invoke(client, state: dict, runtime: dict, session_id: str,
           payload: dict, label: str) -> tuple[dict, str]:
    ...
    for attempt in range(1, CAPACITY_ATTEMPTS + 1):
        ...
        try:
            response = client.invoke_agent_runtime(
                agentRuntimeArn=runtime["arn"], qualifier="DEFAULT",
                runtimeSessionId=sid, payload=json.dumps(payload).encode())
            ...
            if "Insufficient EC2 capacity" in message:
                az = (message.split("requested (")[-1].split(")")[0]
                      if "requested (" in message else "?")
                print(f"    {label}: no {state['instance_type']} capacity in {az} "
                      f"after {elapsed:.0f}s - retry {attempt}/{CAPACITY_ATTEMPTS} "
                      f"on a fresh session")
                time.sleep(5)
                continue
            if code in ("InternalServerException", "RetryableConflictException",
                        "ThrottlingException"):
                print(f"    {label}: {code} after {elapsed:.1f}s - "
                      f"retry {attempt}/{CAPACITY_ATTEMPTS}")
                time.sleep(5 * attempt)
                continue
            raise
    sys.exit(f"{label}: no capacity after {CAPACITY_ATTEMPTS} attempts. GPU capacity "
    ...
    for art in body.get("artifacts") or []:
        ...
        if uri:
            ...
            try:
                ...
                print(f"    ! s3 download of {art['name']} failed: "
                      f"{exc.response['Error']['Code']}; trying the presigned URL")
        url = art.get("url")
        if not url:
            continue
        try:
            with urllib.request.urlopen(url, timeout=300) as r, open(dest, "wb") as fh:  # noqa: S310
                fh.write(r.read())
            saved.append(dest)
...
def report(step: str, body: dict, out_dir: Path | None = None,
    ...
    print(f"    host     : {host.get('hostname')} "
          f"(process {host.get('process_id')}, {host.get('architecture')})")
    ...
    if render:
        ...
        print(f"    rendered : {render.get('device')} in {t.get('generate_s')}s "
    ...
    if body.get("targets_met"):
        tm = body["targets_met"]
        print(f"    targets  : loudness {'met' if tm['loudness'] else 'MISSED'}, "
    ...
    if "validation_passed" in body:
        label = {"cleared": "CLEARED", "review_required": "REVIEW REQUIRED",
                 "not_cleared": "NOT CLEARED"}.get(
                     body.get("outcome"),
                     "CLEARED" if body["validation_passed"] else "NOT CLEARED")
    ...
    if body.get("model_stack"):
        ms = body["model_stack"]
        print(f"    stack    : ready={ms.get('ready')} "
...
def main() -> None:
    ...
    parser.add_argument("--resume", action="store_true",
                        help="stop the session recorded by the previous run and resume "
                             "it, running only the compliance step")
    parser.add_argument("--duration", type=float, default=30.0,
                        help="seconds of audio to render")
    ...
    if args.resume:
        ...
        if not (session_id and track):
            sys.exit("--resume needs a previous run. Run 'python scripts/invoke.py' "
    ...
    if args.resume:
        ...
        print(f"The first invoke provisions a {state['instance_type']} and builds the "
    ...
    if args.resume:
        ...
        for name, runtime in runtimes.items():
            try:
                client.stop_runtime_session(agentRuntimeArn=runtime["arn"],
                ...
                print(f"    {name}: "
                      f"{'nothing to stop' if code == 'ResourceNotFoundException' else code}")
        ...
        #    at deploy time.
        body, active = invoke(client, state, runtimes["composition"], active,
                              {"mode": "prepare"}, "prepare")
        hosts["prepare"] = report("1. prepare model stack (GPU instance + torch + weights)",
                                  body)
        if body.get("status") != "ok":
        ...
        if args.with_catalogue:
            body, active = invoke(client, state, runtimes["composition"], active,
                                  {"mode": "catalogue", "track_id": track,
                                   "duration_s": min(args.duration, 20.0)},
                                  "catalogue")
        ...
        compose_payload: dict = {
            "mode": "compose", "track_id": track,
            "prompt": "Create an upbeat electronic track with heavy bass and synth melodies.",
            "duration_s": args.duration, "seed": 42,
        }
        if args.with_catalogue:
            compose_payload.update(imitate_catalogue="catalogue_00.wav",
                                   reference_strength=0.85,
                                   prompt="Create a melodic techno track with analog "
                                          "bass and warm pads, close to our catalogue sound.")
        body, active = invoke(client, state, runtimes["composition"], active,
                              compose_payload, "compose")
        hosts["composition"] = report(f"{step}. compose (renders audio on the GPU)",
                                      body, run_dir, region)
        step += 1

        # 3. Master, reading the audio the composition agent rendered.
        body, active = invoke(client, state, runtimes["mastering"], active,
                              {"track_id": track, "platform": "spotify",
                               "prompt": "Master this for streaming."}, "master")
        hosts["mastering"] = report(f"{step}. master (real DSP, verified by measurement)",
                                    body, run_dir, region)
        step += 1

    body, active = invoke(client, state, runtimes["compliance"], active,
                          {"track_id": track,
                           "prompt": "Screen this master for release."}, "compliance")
    hosts["compliance"] = report(
        "compliance (resumed session)" if args.resume else "5. compliance screen",
        body, run_dir, region)
    ...
    if body.get("status") == "ok" and body.get("master_is_stale"):
        ...
        body, active = invoke(client, state, runtimes["mastering"], active,
                              {"track_id": track, "platform": "spotify",
                               "prompt": "Master the replacement for streaming."},
                              "re-master")
