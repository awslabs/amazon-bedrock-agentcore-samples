...
from config import FLASK_HOST, FLASK_PORT, FLASK_DEBUG, OUTPUT_DIR, DEFAULT_USER_ID, get_session_id
import os
import sys
import logging
...
def recommend_style():
    ...
    try:
        ...
        if memory_agent:
            recommendations = memory_agent.recommend_style_tool(topic, audience, context)
            return jsonify({
                'success': True,
...
def delete_memory():
    ...
    try:
        if memory_session_manager and memory_agent and memory_session:
                ...
                try:
                    ...
                    if record_id:
                        ...
                        if hasattr(memory_session, 'delete_memory_record'):
                            ...
                            memory_id = getattr(memory_session, '_memory_id', 'SlideDeckAgentMemory-rMV28tDfXu')
                            memory_session_manager.delete_memory_record(
                                memory_id=memory_id,
                                memory_record_id=record_id
                            )
                        else:
                            logger.warning(f"No delete method found, record ID: {record_id}")
                            failed_count += 1
                            continue
                        ...
                        logger.warning(f"No valid ID found in memory record: {list(memory_record.keys())}")
                        failed_count += 1
            ...
            logger.info(f"✅ Successfully deleted {successful_count} memory records for user {user_id}")
            if failed_count > 0:
                logger.warning(f"⚠️ Failed to delete {failed_count} records")

            return jsonify({
                'success': True,
                'message': (f'Successfully deleted {successful_count} preference records! '
                            'The agent will start learning fresh.'),
                'details': {
                    'deleted': successful_count,
                    'failed': failed_count,
                    'namespace': namespace,
                    'deleted_ids': deleted_ids[:5]  # Show first 5 IDs for reference
                }
            })
        else:
            ...
            for filename in os.listdir(OUTPUT_DIR):
                if filename.endswith('.html'):  # Only show HTML files
                    ...
                    file_info = {
                        ...
                        'agent_type': 'Memory Agent' if '_Memory' in filename else 'Basic Agent'
                    }
