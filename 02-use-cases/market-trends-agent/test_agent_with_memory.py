    ...
    try:
        ...
        if "response" in response:
            ...
            followup_response = client.invoke_agent_runtime(
                agentRuntimeArn=agent_arn, payload=json.dumps(followup_payload).encode("utf-8")
            )

            if "response" in followup_response:
                followup_text = followup_response["response"].read().decode("utf-8")
                logger.info("✅ Follow-up response received!")
                logger.info(f"📥 Response: {followup_text[:500]}")
