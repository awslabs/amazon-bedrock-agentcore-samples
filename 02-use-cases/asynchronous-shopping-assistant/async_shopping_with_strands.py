...
fronting_agent = Agent(name="fronting_assistant", 
                    system_prompt="""You are a shopping assistant for amazon.com. You receive a request from the user, and answer immediately
                     if it is a generic question, or route to a background shopping agent. If you decide to allow the shopping agent to start a background 
                     shopping you must return `shop_background.start` in your text response. Once shopping tasks are completed, you may also read any reports or results
...
shopping_agent = Agent(name="shopping_assistant", 
                    system_prompt="""You are a background shopping assistant. You receive a request from the fronting agent who is talking to the customer, and
                     you asynchronously search amazon.com using the call_browser_tool and report back to the customer. Once you start a shopping session, 
                     recognize that this will take a long time to complete. After starting one or more sessions in parallel,
                     return immediately to the user with an appropriate message. To confirm if a session is already running
                     use the get_tasks_info tool. 
                     
                     NOTE 1: When you get a request, the shopping session has not been started yet (check/confirm with get_tasks_info), you need to start one immediately (call_browser_tool).
                    ...
                    tools = [call_browser_tool, get_tasks_info],
                    model = sonnet)
