"""
Evaluate the OpenAI Agents HR Assistant with AgentCore Evaluations.
...
agent_config.json here is built with the OpenAI Agents SDK (see
openai_hr_assistant.py) and instrumented with the OpenTelemetry OpenAI Agents
library (scope opentelemetry.instrumentation.openai_agents).
...
Prerequisites:
    1. Deploy the OpenAI Agents HR Assistant:
...
parser = argparse.ArgumentParser(description="Evaluate the OpenAI Agents HR Assistant")
...
print("=" * 60)
print("OpenAI Agents HR Assistant — AgentCore Evaluation")
print("=" * 60)
...
_resp_quality = _cp.create_evaluator(
    evaluatorName=f"HRResponseQuality_openai_{_SUFFIX}",
    level="TRACE",
    evaluatorConfig={
        "llmAsAJudge": {
            "instructions": (
...
_session_check = _cp.create_evaluator(
    evaluatorName=f"HRSessionCompleteness_openai_{_SUFFIX}",
    level="SESSION",
    evaluatorConfig={
        "llmAsAJudge": {
            "instructions": (
...
# ============================================================
#
# The OpenAI Agents HR assistant is already deployed (deploy.py).
...
SESSION_ID = f"openai-hr-eval-{uuid.uuid4()}"
...
ONLINE_EVAL_CONFIG_NAME = f"hr_openai_eval_{_SUFFIX}"
...
_online_session = f"online-openai-{uuid.uuid4()}"
_online_prompts = [
