"""Deploy the LlamaIndex HR Assistant to AgentCore Runtime using the bedrock-agentcore SDK.
...
Deployment steps:
  ...
  3. Package llamaindex_hr_assistant.py + ARM64 dependencies into a zip
...
The runtime uses a Bedrock model (Nova Lite) via BedrockConverse, so the
bedrock:InvokeModel* permission granted below is sufficient.
...
_AGENT_FILE = "llamaindex_hr_assistant.py"

# Bundled into the deployment zip (ARM64). The full llama-index meta-package is
# required (not just llama-index-core): ADOT's auto-instrumentation checks the
# instrumentation package's declared "instruments" dependency (llama-index) and
# skips the instrumentor if it is missing. llama-index-llms-bedrock-converse
# provides the LLM.
_PACKAGES = [
    "llama-index",
...
parser = argparse.ArgumentParser(description="Deploy the LlamaIndex HR Assistant to AgentCore Runtime")
...
_AGENT_NAME = f"hr_llamaindex_{uuid.uuid4().hex[:8]}"
