"""
Evaluate the Google ADK HR Assistant with AgentCore Evaluations.
...
parser = argparse.ArgumentParser(description="Evaluate Google ADK HR Assistant")
...
print("=" * 60)
print("Google ADK — AgentCore Evaluation")
print("=" * 60)
...
_resp_quality = cp_client.create_evaluator(
    evaluatorName=f"HRResponseQuality_google_adk_{_SUFFIX}",
    level="TRACE",
    evaluatorConfig={
        "llmAsAJudge": {
            "instructions": (
...
print("\n[2/4] Invoking Google ADK HR Assistant ...")

SESSION_ID = f"google-adk-eval-{uuid.uuid4()}"
...
ONLINE_CONFIG_NAME = f"hr_google_adk_eval_{_SUFFIX}"
...
print("\n" + "=" * 60)
print("Evaluation Complete — Google ADK")
print("=" * 60)
