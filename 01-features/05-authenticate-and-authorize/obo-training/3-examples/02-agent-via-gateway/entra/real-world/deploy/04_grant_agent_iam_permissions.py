...
POLICY_NAME = "obo-uc2-entra-agent-obo-permissions"
...
def find_agent_role_via_cfn(agent_runtime_name: str, region: str) -> str | None:
    ...
    The AgentCore CLI creates a stack named `AgentCore-<AGENT_RUNTIME_NAME>-default`
    and inside it an IAM role whose LogicalResourceId contains 'ExecutionRole'.
    CloudFormation truncates the physical role name to stay under IAM's 64-char
    limit, which is why substring-matching on the physical name is unreliable.
    """
...
def find_agent_role_via_iam_scan(iam, agent_runtime_name: str) -> list[str]:
    ...
    Uses truncated-prefix matching because CFN often truncates the runtime name
    when composing the physical role name.
    """
    # Try progressively shorter prefixes (min 8 chars) to survive CFN truncation.
...
def find_agent_role(iam, agent_runtime_name: str, region: str) -> str:
    ...
    Resolution order:
      ...
      2. CloudFormation stack lookup (most reliable — the stack name is deterministic).
    ...
    return {
        ...
        "Statement": [
            ...
            },
            {
                ...
                # the directory ARN must be here. Observed 404 in testing:
                #   "no identity-based policy allows the
                #    bedrock-agentcore:GetResourceOauth2Token action on resource:
                #    workload-identity-directory/default"
                # when only the /workload-identity/<name> child was listed.
                "Resource": [
                    workload_directory_arn,
                    workload_identity_arn,
                    token_vault_arn,
...
def main() -> None:
    ...
    # find_agent_role() honors AGENT_EXECUTION_ROLE_NAME internally.
    ...
    print()
    print("Next: copy the agent runtime ARN from `agentcore status` into")
    print("AGENT_RUNTIME_INVOKE_URL in .env, then run python frontend/app.py")
