...
per hop in chronological order — with Okta client IDs annotated with
friendly labels sourced from .env.
...
Prerequisites:
  - AGENT_RUNTIME_NAME is set in .env.
...
# .env keys that hold Okta client IDs we can label in the trace output.
...
# Well-known values worth labeling if they show up in tokens.
STATIC_LABELS = {
    "https://httpbin.org/anything": "httpbin.org/anything (mock downstream)",
    "https://httpbin.org": "httpbin.org (mock downstream)",
}
...
def main() -> None:
    ...
    if not (runtime_dir / "agentcore" / "agentcore.json").exists():
        print(
            ...
            "       Has the agent been scaffolded + deployed? See README.md.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Build a substitution map: known Okta client IDs -> friendly labels.
