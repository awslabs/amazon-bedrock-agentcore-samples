"""
Tear down the Use Case 2 (Okta) AgentCore resources.
...
Deletes (in reverse-dependency order):
  ...
  - The Gateway service IAM role (only if it matches our auto-create name
    or is set in .env — we never delete a role we can't attribute to this
    example).
...
What it does NOT touch:
  - Okta app registrations. Use `deploy/00_delete_okta_apps.py`.
...
def delete_all_targets(client, gateway_id: str) -> None:
    """Delete every target on the gateway.
...
def clean_env_values(env_path: Path) -> None:
    """Clear values of deploy-populated keys, leaving the keys themselves in place."""
...
def main() -> None:
    ...
    if survivors:
        ...
        print()
        print("✓ All AWS resources for UC2 Okta are cleaned up.")
