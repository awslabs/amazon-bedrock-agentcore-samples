"""
Enable + inspect observability for UC2 Okta: Runtime and Gateway.
...
resource. This script complements that with:
  ...
  2. Prints ready-to-run diagnostic commands for the runtime + gateway.
...
def find_uc2_log_groups(logs, agent_runtime_name: str, gateway_name: str) -> list[dict]:
    ...
    for page in paginator.paginate(logGroupNamePrefix="/aws/bedrock-agentcore/"):
        for lg in page.get("logGroups", []):
            ...
            hay = name.lower()
            # Truncated CFN names surface as a prefix — match on the first
            # 15 chars of the runtime name to be robust against that.
...
def main() -> None:
    ...
    print()
    print(f"  Tail agent runtime logs (warn+ only, useful when debugging OBO 401s):")
    ...
    print()
    print(f"  Use the trace helper — cleaner than raw logs:")
    print(f"    python deploy/show_obo_trace.py --since 10m")
    print()
