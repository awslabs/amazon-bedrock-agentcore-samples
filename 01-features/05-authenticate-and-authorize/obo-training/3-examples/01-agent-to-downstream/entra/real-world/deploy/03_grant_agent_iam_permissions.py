...
Permissions added:
  - bedrock-agentcore:GetWorkloadAccessToken{,ForJWT,ForUserId}
    on the workload identity directory + the specific workload for this agent.
  - bedrock-agentcore:GetResourceOauth2Token
    on the token vault and credential providers.
  - secretsmanager:GetSecretValue
    on the AgentCore Identity-managed OAuth2 credential secrets.
...
def _find_agent_execution_role(cfn, agent_runtime_name: str) -> str:
    ...
    print(
        ...
        f"inside stack {stack_name}. Resources present:",
        file=sys.stderr,
    )
