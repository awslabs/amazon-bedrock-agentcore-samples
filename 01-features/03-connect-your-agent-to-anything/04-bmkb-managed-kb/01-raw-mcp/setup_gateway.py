"""Provision an IAM gateway role + an AgentCore Gateway + an BMKB Retrieve target.

Writes resource ids to ../.env.bmkb-gateway so 02-strands-agent can pick them up.
...
def get_or_create_gateway_role(name: str, account_id: str, region: str, kb_id: str) -> str:
    ...
    try:
        iam.create_role(
            ...
            Description="AgentCore Gateway role for BMKB Retrieve",
        )
...
def main() -> int:
    ...
    p.add_argument("--kb-id", required=True, help="Managed KB id (BMKB)")
    p.add_argument("--region", default="us-west-2")
    p.add_argument("--name-prefix", default="bmkb-sample", help="prefix for the gateway role / gateway / target")
    args = p.parse_args()
    ...
    env_path = ROOT / ".env.bmkb-gateway"
    env_path.write_text(
    ...
    print(f"\nwrote {env_path}")
    print("source it:  source .env.bmkb-gateway")
    return 0
