...
- The [A2A agent on AgentCore Runtime](../../../01-attach-targets/http/agents/a2a-agents/agentcore-runtime/) lab deployed end to end (the gateway, the runtime, and the OBO target). This tutorial reuses that lab's `scripts/a2a-runtime-target/.env` and its `RUNTIME_ARN` / `BEARER_TOKEN` exports.
...
Before hardening, a direct call to the runtime invocation URL succeeds with a valid bearer token (the same `BEARER_TOKEN` from the A2A lab). This is the path the hardening closes.
...
The direct call returns an answer, demonstrating that the runtime accepts any valid token regardless of whether the gateway was involved.
...
Wait for the runtime to return to `READY`:
...
Re-run the **direct** call from Step 2. It now fails, because the request reaches the runtime without the gateway in its identity chain:
...
Expect a `401` or `403`. The call **through the gateway** still works, since that request carries the allowed gateway in its identity chain:
