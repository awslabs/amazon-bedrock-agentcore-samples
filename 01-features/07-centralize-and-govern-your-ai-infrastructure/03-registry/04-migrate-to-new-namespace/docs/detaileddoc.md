...
| Two source records share one name | One record keeps the name; the other is refused and reported, so subsequent runs behave identically until the source is corrected. With `duplicateNames = "suffix"`, the other record is loaded under a distinct name and keeps that name on every later run. See [How `suffix` chooses which record is renamed](#how-suffix-chooses-which-record-is-renamed). |
...
Alongside each ID, the map records the `name` (and `recordVersion`) the record was migrated under. That is what makes `duplicateNames = "suffix"` stable across runs, as described next.

> **Note**
> Dry runs never write to the ID map, because they create no records.

### How `suffix` chooses which record is renamed

With `duplicateNames = "suffix"`, a set of source records that transform to the same target `name` and `recordVersion` is resolved before any record in the run is written. Exactly one record keeps the shared name; each of the others is loaded as `<name>-<8 hex>`, where the hex is a digest of that record's own source identity (`<accountId>/<region>/<registryId>/<recordId>`). The renamed record keeps the original name in its `displayName`, and the crosswalk row shows both (`previewName` and `name`).

Which record keeps the shared name is decided in this order:

1. **A record already in the target registry keeps the name it is there under.** This includes records the current run does not carry — an incremental run reads the names of every previously migrated record from the ID map, so it cannot hand one of them to a different record.
2. **Otherwise, the record with the lowest source identity keeps the name**, comparing `<accountId>/<region>/<registryId>/<recordId>` as text.

Neither rule can be influenced by how many records a run carries or by the order in which the preview API returned them, so the same set of source records resolves the same way in every run, in either execution mode. A record that has been renamed away once keeps its suffixed name even if the record it collided with is later deleted from the source registry; renaming it back would break every reference to a published name.

Three consequences to be aware of:

- **The renamed record is whichever the rules select, not "the second one".** If two records collide and the one with the higher source identity is migrated first — for example, because an incremental window carried only that one — that record keeps the shared name, and the record with the lower identity is the one renamed when it arrives.
- **`suffix` reduces collisions that stop a migration, but does not remove them.** If both the shared name and a record's own suffixed form are already held by other records, that record still fails and is reported (`… and its distinct name … is held by …`). Fixing the source registry is the durable answer.
- **A record whose own name is another record's suffixed form keeps its own name.** The suffixed names a run hands out are reserved alongside the names records asked for, and a record asking for a name under its own steam outranks one being moved onto it — so the record carrying the coincidental name is loaded under it, and the record left with nowhere to go fails as above rather than the two of them being separated by whichever the load reached first.
...
| `… both migrate to the target registry name …` | The preview registry permitted duplicate record names. The new version requires `name` and `recordVersion` to be unique, and migrated records retain their preview names, so loading the second would overwrite the first. Only that record failed: the rest of the run loaded, and the run reports `PARTIAL_SUCCESS`. | The error names both source records. In the preview registry, rename all but one record in each set or assign distinct `recordVersion` values, then re-run. If the source cannot be modified, set `runtime.transform.duplicateNames` to `"suffix"` and re-run (both stages, since the setting is part of the replay fingerprint). |
| A record's warnings say `… does not belong to this record, and duplicateNames is 'suffix', so it was migrated as …` | `duplicateNames = "suffix"` is set, and this record is not the one entitled to the name it came with, so it was renamed. | This is expected behavior when that setting is enabled. The warning gives the name to look the record up by; the crosswalk maps `previewName` to `name`, and `displayName` retains the preview name. See [How `suffix` chooses which record is renamed](#how-suffix-chooses-which-record-is-renamed). |
| `… and its distinct name … is held by …` | `duplicateNames = "suffix"` is set, but both the name this record would take and its own suffixed form are already held by other source records. | `suffix` moves a record off a name another record holds; it cannot move it anywhere when both names are taken. Only that record failed. Rename it in the preview registry or give it a distinct `recordVersion`, then re-extract. |
