...
def render_report(report: dict[str, Any], extract_summary: dict[str, Any] | None = None) -> str:
    ...
    parts = [
        ...
        "<footer>Generated by the AWS Agent Registry Preview to GA migration tool from "
...
def _attention_sentence(attention: int) -> str:
    ...
    plural = attention != 1
    return (
        f"{attention} check{'s' if plural else ''} "
        f"need{'' if plural else 's'} your attention -- see below."
    )


def _render_load_hero(report: dict[str, Any], status: str, dry_run: bool, attention: int) -> str:
    """The one thing to read first: what this run did to GA, and whether anything needs attention.

    States the outcome in records rather than opening with a count of internal checks: a reviewer's
    first question is "what is in GA now, and what is missing", not "how many checks scored".
    """
    totals = _load_totals(report)
    failed = totals["failed"]
    in_ga = totals["created"] + totals["updated"] + totals["existing"]

    if dry_run:
        headline = "Dry run -- nothing was written to GA"
        would = totals["dryRun"]
        sentences = [
            f"A live run of this extract would create {would} record(s) in your GA registries."
            if would
        ...
        if in_ga and failed:
            headline = f"{in_ga} of {totals['extracted']} records are now in your GA registries"
        elif in_ga:
            headline = f"All {in_ga} records are now in your GA registries"
        elif failed:
            headline = "Nothing was written to GA -- every record failed"
        else:
            headline = "This attempt wrote no records to GA"
        sentences = []
        if failed:
            sentences.append(
                f"{failed} record(s) failed and were not created in GA. "
...
def _render_extract_hero(ready: bool, attention: int, staged: int = 0) -> str:
    ...
    if not ready:
        return (
            ...
            '<p class="sub">Nothing has been written to GA. Loading this extract could migrate only '
    ...
    if attention:
        return (
            ...
            f'<p class="sub">Extraction only reads; nothing has been written to GA yet. '
            f"{_e(_attention_sentence(attention))}</p></div></div>"
        )
    return (
        f'<div class="hero ok"><div class="icon">&#10003;</div><div>'
        f'<p class="headline">Ready to load -- {staged} record(s) staged</p>'
        f'<p class="sub">Extraction only reads; nothing has been written to GA yet. Confirm the '
...
def _render_extract_stats(extract_summary: dict[str, Any]) -> str:
    ...
    stats = "".join(
        f'<div class="stat {variant}"><span class="n">{_e(n)}</span>'
...
def _render_extract_registries(extract_summary: dict[str, Any]) -> str:
    header = (
        "<tr><th>Registry pair</th><th>Source</th><th>Target</th><th>Status</th>"
        "<th>Records</th><th>Warnings</th></tr>"
    )
    rows = []
    ...
    checks.append(
        _check(
            ...
            (
                ...
                else f"Only {accounted} of {extracted} extracted record(s) reached a final "
                f"outcome: {outcomes}."
            ),
            ""
            if accounted == extracted
    ...
    if not dry_run and registries:
        checks.append(
            _check(
                ...
                "independently count every record currently stored in Preview and GA.",
                "After resolving any failures, list each GA target and confirm it contains every "
                "expected Preview record. The GA total can be higher if it already contains "
                "unrelated records: aws agent-registry-control list-registry-records "
                "--registry-id <ga-registry-id>",
            )
        )
    ...
    if types:
        checks.append(
            _check(
                ...
                "shape. Two of these names change in GA: A2A becomes AGENT, and AGENT_SKILLS "
                "becomes SKILL. MCP and CUSTOM keep their names. Confirm these totals are the "
                "split you expect before you rely on filtering by type in GA; if one looks wrong, "
...
def _render_reference_notes() -> str:
    ...
    notes = [
        (
            ...
            "The GA descriptor shape differs from preview by design. Spot-check a few records in "
            "the record-comparison artifact below -- each entry has the preview record, the "
            "transformed payload, and the resulting GA record.",
        ),
        (
            "Every record has a new recordId",
            "GA records are created fresh. Anything referencing a preview recordId -- client "
            "configuration, infrastructure as code, links between records -- has to be repointed "
            "using the crosswalk below.",
        ),
        (
            "The service namespace changes",
            "bedrock-agentcore becomes agent-registry. Update IAM policies, endpoints, SDK "
            "clients, CLI commands and resource ARNs, then confirm your applications can read and "
            "write through the new namespace.",
        ),
    ]
    items = "".join(
        f'<div class="item"><span class="what">{_e(what)}.</span> {_e(detail)}</div>'
...
def _render_meta(report: dict[str, Any], status: str, dry_run: bool) -> str:
    badge = {"SUCCEEDED": "succeeded", "FAILED": "failed", "PARTIAL_SUCCESS": "partial"}.get(
        status, "partial"
    )
    mode = (
...
def _render_registries(report: dict[str, Any]) -> str:
    header = (
        "<tr><th>Registry pair</th><th>Preview source</th><th>GA target</th>"
...
def _render_status_section(report: dict[str, Any]) -> str:
    ...
    ga = approval.get("gaStatusCounts") or {}
    statuses = sorted(set(source) | set(ga))
    rows = "".join(
        "<tr>"
        f"<td>{_e(status)}</td>"
        f'<td class="num">{_e(source.get(status, 0))}</td>'
        f'<td class="num">{_e(ga.get(status, 0))}</td>'
        "</tr>"
        for status in statuses
    )
    return (
        "<h2>Approval status</h2>"
        "<table><tr><th>Status</th><th>In preview</th><th>In GA</th></tr>"
        f"{rows}</table>"
    ...
    for registry in failures:
        ...
        for detail in compact_details:
            ...
            if detail.get("newRecordId"):
                identifiers.append(f"GA ID: {_e(detail['newRecordId'])}")
            record = _e(name)
            if identifiers:
