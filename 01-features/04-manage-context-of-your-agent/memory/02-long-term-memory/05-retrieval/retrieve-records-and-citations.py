...
Three surfaces:
    python retrieve-records-and-citations.py boto3      # raw boto3 (bedrock-agentcore / -control)
    python retrieve-records-and-citations.py sdk        # AgentCore SDK, low-level MemoryClient
    python retrieve-records-and-citations.py session    # AgentCore SDK, high-level MemorySessionManager
...
def run_with_session(cleanup: bool = False) -> None:
    ...
    print(f"\n[session] Semantic search 'dietary restrictions' ({len(semantic)}):")
    ...
    print(f"\n[session] ListLongTermMemoryRecords ({len(listed)}):")
    for h in listed:
        print(f"  - {h['memoryRecordId']}: {h['content']['text']}")

    if listed:
        full = session.get_memory_record(listed[0]["memoryRecordId"])
        print("\n[session] GetMemoryRecord (one):")
        print(f"  id={full['memoryRecordId']}")
        print(f"  text={full['content']['text']}")
        print(f"  createdAt={full.get('createdAt')}")
