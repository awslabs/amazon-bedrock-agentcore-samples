...
- Step 4 (writing the `default` target) is a manual step because no
  ...
  See [`NOTES.md`](NOTES.md). May change in a later CLI release.
- The CLI's harness supports `bedrock`, `open_ai`, and `gemini`. Mantle and
  LiteLLM are not in this CLI version yet.
