...
def test_tool_organization(all_tools: list) -> list:
    ...
    if len(travel_tools) >= 3:
        ...
        print(f"   {Fore.RED}✗ Expected at least 3 travel tools, found {len(travel_tools)}{Style.RESET_ALL}")
        results.append(False)
    ...
    if len(itinerary_tools) >= 3:
        ...
        print(f"   {Fore.RED}✗ Expected at least 3 itinerary tools, found {len(itinerary_tools)}{Style.RESET_ALL}")
        results.append(False)
