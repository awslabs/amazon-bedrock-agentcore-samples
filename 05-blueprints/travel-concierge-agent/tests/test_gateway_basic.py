...
For tool execution tests, see test_gateway_comprehensive.py.

Usage:
    uv run --with boto3 --with requests --with colorama --with strands-agents --with mcp \
        python tests/travel/test_gateway_basic.py
"""
...
def call_gateway(
    gateway_url: str, access_token: str, method: str, params: dict = None
) -> dict:
...
def test_tool_organization(all_tools: list) -> list:
    ...
    if len(cart_tools) >= 5:
        ...
        print(
            f"   {Fore.RED}✗ Expected at least 5 cart tools, found {len(cart_tools)}{Style.RESET_ALL}"
        )
    ...
    if len(travel_tools) >= 3:
        print(
            f"   {Fore.GREEN}✓ Found {len(travel_tools)} travel tools{Style.RESET_ALL}"
        )
        results.append(True)
    else:
        print(
            f"   {Fore.RED}✗ Expected at least 3 travel tools, found {len(travel_tools)}{Style.RESET_ALL}"
        )
        results.append(False)
    ...
    if len(itinerary_tools) >= 3:
        print(
            f"   {Fore.GREEN}✓ Found {len(itinerary_tools)} itinerary tools{Style.RESET_ALL}"
        )
        results.append(True)
    else:
        print(
            f"   {Fore.RED}✗ Expected at least 3 itinerary tools, found {len(itinerary_tools)}{Style.RESET_ALL}"
        )
        results.append(False)
...
def test_mcpclient_tool_filters(gateway_url: str, access_token: str) -> list:
    ...
    def test_filter(name: str, filter_pattern: str, expected_min: int):
        ...
        try:
            client = create_gateway_mcp_client(
                access_token, gateway_url, filter_pattern
            )
            ...
            with client:
                ...
                if len(tools) >= expected_min:
                    ...
                    print(
                        f"   {Fore.RED}✗ Expected at least {expected_min}, got {len(tools)}{Style.RESET_ALL}"
                    )
                    return False
    ...
    results.append(test_filter("Travel Subagent", "^traveltools___", 3))
    results.append(test_filter("Supervisor (Itinerary)", "^itinerarytools___", 3))
