...
class ToolCall:
    ...
    tool_use_id: str = ""
    timestamp: float = 0.0
    nested_tools: List[NestedToolCall] = field(
        default_factory=list
    )  # Full nested tool details
...
class AgentTrajectoryTester:
    ...
    def __init__(
        self, config: Dict, verbose: bool = False, subagent_names: List[str] = None
    ):
        self.config = config
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if row.get("expected_nested_tools"):
                    nested = [
                        t.strip() for t in row["expected_nested_tools"].split(",")
                    ]
    ...
    def _parse_streaming_response(self, response) -> Tuple[str, List[ToolCall]]:
            ...
            try:
                ...
                if data.get("type") == "tool_stream":
                    ...
                    if (
                        ...
                        and "current_tool_use" in inner_data
                    ):
                                ...
                                if tool_calls:
                                    ...
                                    if existing:
                                        ...
                                        if (
                                            ...
                                            and len(str(nested_input))
                                            > len(str(existing.input_args))
                                        ):
                                            existing.input_args = nested_input
                                    else:
                                        # Add new nested tool
                                        tool_calls[-1].nested_tools.append(
                                            NestedToolCall(
        ...
        unique = {}
        for tc in tool_calls:
            if tc.tool_name not in unique or (
                tc.input_args and not unique[tc.tool_name].input_args
            ):
    ...
    def validate_tool_calls(
        self, actual: List[ToolCall], expected: List[Dict]
    ) -> Tuple[bool, List[str]]:
    ...
    def run_turn(self, turn_data: Dict, user_id: str, session_id: str) -> TurnResult:
        ...
        try:
            ...
            passed, errors = self.validate_tool_calls(
                tool_calls, result.expected_tool_calls
            )
    ...
    def _print_comparison(self, turn_data: Dict, result: TurnResult):
        ...
        if result.passed:
            print(
                f"     {Fore.GREEN}✓ PASSED ({result.duration:.2f}s){Style.RESET_ALL}"
            )
        else:
    ...
    def run_all(
        self, sessions: List[Dict], filter_session: Optional[str] = None
    ) -> List[SessionResult]:
        ...
        for session in input_sessions:
            for turn in session["turns"]:
                ...
                expected_lookup[key] = {
                    ...
                    "expected_tools": turn["expected_trajectory"].get(
                        "tool_sequence", []
                    ),
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                [
                    ...
                    "expected_agent",
                    "expected_tools",
                    "expected_tools_input",
                    "actual_tools",
                    "actual_tools_input",
                    "actual_tools_output",
                    "expected_nested_tools",
                    "actual_nested_tools",
                    "nested_tools_match",
                    "all_tool_details",
                    "expected_answer",
            ...
            for session in self.results:
                for turn in session.turns:
                    ...
                    exp_tools_input = json.dumps(
                        [
                            {"tool": t["tool"], "input": t.get("input", {})}
                            for t in exp_tools
                        ]
                    )
                    ...
                    act_tools_input = json.dumps(
                        [
                            {"tool": t.tool_name, "input": t.input_args}
                            for t in act_tools
                        ]
                    )
                    act_tools_output = json.dumps(
                        [
                            {"tool": t.tool_name, "output": (t.output or "")[:500]}
                            for t in act_tools
                        ]
                    )
                    ...
                    if exp_nested:
                        missing = [
                            t for t in exp_nested if t not in turn.actual_nested_tools
                        ]
                        nested_match = (
                            "MATCH" if not missing else f'MISSING: {",".join(missing)}'
                        )
                    ...
                    writer.writerow(
                        [
                            ...
                            session.user_id,
                            turn.turn,
                            turn.test_description,
                            turn.question,
                            "PASS" if turn.passed else "FAIL",
                            f"{turn.duration:.2f}",
                            turn.expected_agent,
                            exp_tools_names,
                            exp_tools_input,
                            ...
                            act_tools_output,
                            ",".join(exp_nested),
                            ",".join(turn.actual_nested_tools),
                            nested_match,
                            all_details,
                            expected.get("expected_answer", "")[:500],
...
def main():
    ...
    parser.add_argument(
        "--subagents",
        type=str,
        help="Comma-separated subagent names to exclude from nested tools",
    )
    ...
    if args.subagents:
        ...
        # Default for travel agent (can be overridden)
        subagent_names = ["travel_assistant", "cart_manager"]
    ...
    # Load dataset
    tester = AgentTrajectoryTester(
        config, verbose=args.verbose, subagent_names=subagent_names
    )
