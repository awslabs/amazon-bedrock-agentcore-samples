...
def add_flight_to_cart(user_id: str, flights: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        # Validate required fields BEFORE adding
        required_fields = ['title', 'price', 'flight_id', 'origin', 'destination', 'departure_date']
    ...
    try:
        ...
        for item in cart_items:
            ...
            try:
                ...
                logger.warning(f"Could not parse price '{item.get('price', '0')}' for item {item.get('title', 'unknown')}")
...
def onboard_card(
    ...
    try:
        ...
        return {
            'success': False,
            'message': f'Error onboarding card: {str(e)}'
        }
