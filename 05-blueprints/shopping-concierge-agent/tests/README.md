...
Test suite for the Shopping Concierge Agent with MCP tools (Cart, Shopping).
...
| `test_gateway_tools.py` | Confirms all tools return expected responses (CSV-based, 19 tests) |
| `test_agent_trajectories.py` | Validates tool sequence and agent behavior for specific scenarios (18 sessions) |
...
| File | Description |
|------|-------------|
| `gateway_tool_test_dataset.csv` | 19 gateway tool tests (write/verify pattern) |
| `agent_test_dataset.csv` | 18 agent trajectory tests |
...
python tests/test_agent_remote.py -q "Find hiking boots"       # Single query
```
...
### Cart Tools (10)
`get_cart`, `add_to_cart`, `remove_from_cart`, `clear_cart`, `request_purchase_confirmation`, `confirm_purchase`, `onboard_card`, `get_visa_iframe_config`, `check_user_has_payment_card`, `send_purchase_confirmation_email`

### Shopping Tools (2)
`single_productsearch`, `generate_packinglist_with_productASINS`
