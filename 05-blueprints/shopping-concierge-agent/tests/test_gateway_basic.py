...
def test_tool_organization(all_tools: list) -> list:
    ...
    if len(shopping_tools) >= 2:
        print(
            f"   {Fore.GREEN}✓ Found {len(shopping_tools)} shopping tools{Style.RESET_ALL}"
        )
        results.append(True)
    else:
        print(
            f"   {Fore.RED}✗ Expected at least 2 shopping tools, found {len(shopping_tools)}{Style.RESET_ALL}"
        )
        results.append(False)
