...
class GatewayToolTester:
    ...
    def discover_tools(self) -> bool:
        ...
        try:
            resp = requests.post(self.gateway_url, headers=headers, json=payload, timeout=30)
            result = resp.json()
    ...
    def call_tool(self, tool_name: str, args: Dict) -> Tuple[bool, Any, str]:
        ...
        try:
            resp = requests.post(self.gateway_url, headers=headers, json=payload, timeout=60)
            result = resp.json()
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                test_cases.append(TestCase(
                    ...
                    expected_output_contains=row.get('expected_output_contains', ''),
    ...
    def run_test(self, test_case: TestCase) -> TestResult:
        ...
        start = time.time()
        success, result, error = self.call_tool(test_case.tool_name, test_case.input_args)
        duration = time.time() - start
        
        if not success:
            print(f"     {Fore.RED}✗ FAILED ({duration:.2f}s): {error}{Style.RESET_ALL}")
        ...
        if not valid:
            print(f"     {Fore.RED}✗ FAILED ({duration:.2f}s): {validation_error}{Style.RESET_ALL}")
    ...
    def run_all(self, test_cases: List[TestCase], 
                ...
                filter_test_id: str = None) -> List[TestResult]:
    ...
    def save_results_csv(self, output_path: Path):
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'test_id', 'category', 'tool_name', 'test_description', 'action_description',
                'input_json', 'expected_output_contains', 'expected_output_type',
                'validation_rule', 'passed', 'duration_s', 'actual_output', 'error'
            ])
            
            for r in self.results:
                tc = r.test_case
                output_str = json.dumps(r.actual_output)[:1000] if r.actual_output else ""
                
                writer.writerow([
                    tc.test_id, tc.category, tc.tool_name, tc.test_description,
                    tc.action_description, json.dumps(tc.input_args), tc.expected_output_contains,
                    tc.expected_output_type, tc.validation_rule,
                    'PASS' if r.passed else 'FAIL', f"{r.duration:.2f}",
                    output_str, r.error
                ])
...
def main():
    parser = argparse.ArgumentParser(description="Test gateway tools with CSV-based test cases")
    parser.add_argument("--input", "-i", type=str, help="Input CSV file")
    parser.add_argument("--output", "-o", type=str, help="Output CSV file")
    parser.add_argument("--category", "-c", type=str, help="Filter by category (cart, travel, itinerary)")
