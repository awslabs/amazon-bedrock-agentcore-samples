...
def get_amplify_config() -> Dict:
    ...
    if amplify_path.exists():
        with open(amplify_path) as f:
            return json.load(f)
    raise FileNotFoundError("amplify_outputs.json not found. Run 'npm run deploy:amplify' first.")
        ...
        try:
            ...
            if isinstance(data, dict):
                if "data" in data and isinstance(data["data"], str):
                    ...
                    delta_text = data.get("event", {}).get("contentBlockDelta", {}).get("delta", {}).get("text", "")
                    if delta_text:
