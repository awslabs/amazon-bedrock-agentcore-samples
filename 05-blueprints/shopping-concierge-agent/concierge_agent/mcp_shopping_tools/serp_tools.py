    ...
    try:
        ...
        for product in organic_results:
            product_info = {
                ...
                'price': product.get('price', {}).get('value', 0) if isinstance(product.get('price'), dict) else product.get('price', 'N/A'),
...
def generate_packing_list(user_id: str, question: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if any(word in question_lower for word in ["beach", "hawaii", "tropical", "ocean"]):
            packing_items.extend(["sunscreen SPF 50", "beach towel", "swimsuit", "flip flops", "sunglasses"])
        elif any(word in question_lower for word in ["ski", "snow", "winter", "cold"]):
            packing_items.extend(["winter jacket", "thermal underwear", "ski goggles", "gloves", "beanie"])
        elif any(word in question_lower for word in ["hiking", "camping", "outdoor"]):
            packing_items.extend(["hiking boots", "water bottle", "first aid kit", "flashlight", "sleeping bag"])
        elif any(word in question_lower for word in ["business", "work", "conference"]):
            packing_items.extend(["business casual clothes", "laptop bag", "power bank", "notebook"])
        else:
