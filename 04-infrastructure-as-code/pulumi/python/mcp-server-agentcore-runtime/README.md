...
| `ecr_repository_url`          | ECR repository URL for the MCP server image |
| `codebuild_project_name`      | Name of the CodeBuild project               |
| `cognito_user_pool_id`        | Cognito User Pool ID                        |
| `cognito_user_pool_client_id` | Cognito User Pool Client ID                 |
| `cognito_discovery_url`       | OIDC discovery URL for JWT validation       |
| `get_token_command`           | Ready-to-run command to get a JWT token     |
...
```bash
.venv/bin/pip install boto3 mcp
```
...
```bash
CLIENT_ID=$(pulumi stack output cognito_user_pool_client_id -s dev)
pulumi env run <esc-project>/<esc-environment> -- .venv/bin/python get_token.py "$CLIENT_ID" testuser TestPassword123 us-east-1
```
...
pulumi env run <esc-project>/<esc-environment> -- .venv/bin/python test_mcp_server.py "$RUNTIME_ARN" YOUR_JWT_TOKEN
```
