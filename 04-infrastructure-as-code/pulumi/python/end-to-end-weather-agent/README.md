...
| `agent_runtime_id`         | ID of the AgentCore runtime            |
| `agent_ecr_repository_url` | ECR repository URL for the agent image |
| `codebuild_project_name`   | Name of the CodeBuild project          |
| `browser_id`               | Browser tool ID                        |
| `code_interpreter_id`      | Code Interpreter tool ID               |
| `memory_id`                | Memory ID                              |
| `results_bucket_name`      | S3 bucket for agent-generated results  |
| `source_bucket_name`       | S3 bucket for agent source code        |
| `log_group_name`           | CloudWatch Log Group for runtime logs  |
| `test_script_command`      | Ready-to-run command to test the agent |
...
```bash
.venv/bin/python test_weather_agent.py "$(pulumi stack output agent_runtime_arn -s dev)"
```

If you use Pulumi ESC for AWS credentials:

```bash
RUNTIME_ARN=$(pulumi stack output agent_runtime_arn -s dev)
pulumi env run <esc-project>/<esc-environment> -- .venv/bin/python test_weather_agent.py "$RUNTIME_ARN"
```
...
```bash
BUCKET=$(pulumi stack output results_bucket_name -s dev)
...
4. **Code Generation** (LLM):
   ...
   - Classification rules:
     - GOOD: 65-80F, clear conditions, no rain
     - OK: 55-85F, partly cloudy, slight rain chance
     - POOR: <55F or >85F, cloudy/rainy
...
```bash
BROWSER_ID=$(pulumi stack output browser_id -s dev)
