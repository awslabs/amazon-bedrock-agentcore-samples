...
def fetch_access_token(client_id, client_secret, token_url):
    ...
    if response.status_code == 200:
        ...
        print(f"Token request failed. Trying with scopes...")
        # Try with scopes
        data["scope"] = "gateway-api/read gateway-api/write"
        response2 = requests.post(token_url, data=data, headers={'Content-Type': 'application/x-www-form-urlencoded'})
        print(f"With scopes - Status: {response2.status_code}, Response: {response2.text}")
