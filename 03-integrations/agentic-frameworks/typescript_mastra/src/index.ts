...
app.post('/invocations', (req: Request, res: Response) => {
  try {
    const sessionId = req.headers['x-amzn-bedrock-agentcore-runtime-sessionid'] as string;
    const requestId = req.headers['x-amzn-bedrock-agentcore-runtime-requestid'] as string;
    ...
    if (!sessionId) {
      return res.status(400).json({
        error: 'Missing required header: X-Amzn-Bedrock-AgentCore-Runtime-SessionId'
      });
    }
