"""
Deploy Codex agent to AgentCore Runtime via OpenAPI API key using a container image from ECR.
...
OPENAI_SECRET_NAME  = "openai/codex"
OPENAI_SECRET_ARN   = f"arn:aws:secretsmanager:{REGION}:{ACCOUNT_ID}:secret:{OPENAI_SECRET_NAME}"
    ...
    inline_policy = {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [f"arn:aws:ecr:{REGION}:{ACCOUNT_ID}:repository/agentcore-codex"],
            },
            {
                "Sid": "S3Files",
                "Effect": "Allow",
...
def create_runtime(role_arn: str) -> dict:
    ...
    create_params = dict(
        ...
        description="Codex agent on AgentCore Runtime with S3 Files, backed by OpenAI gpt-4o",
    )
